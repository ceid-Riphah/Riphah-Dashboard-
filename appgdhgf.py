...
FACULTY_SHEETS = {
    "Riphah College of Science and Technology": None,
    "Riphah School of Business and Management": None,
    ...
    "Riphah Institute of Pharmaceutical Sciences": None,
    "Riphah Institute of Clinical & Professional Psychology": None,
    "Riphah College of Rehabilitation and Allied Health Sciences": None,
}
...
def render_kpi_section(department: str, vertical_display: str):
    ...
    try:
        with st.spinner("Loading KPI data..."):
            sheet_df = load_faculty_vertical_sheet(sheet_id, vertical_key)
    except Exception as e:
        st.error(
            "Couldn't load this tab. Make sure the spreadsheet is shared as "
            f"'Anyone with the link -> Viewer' and has a tab named exactly "
            f"'{vertical_key}'. Details: {e}"
        )
        return
...
def render_ceid_portal():
    ...
    with nav_col:
        ...
        for dept in CEID_DEPARTMENTS:
            ...
            label = dept + (" ✅" if FACULTY_SHEETS.get(dept) else "")
...
def render_ceo_overview():
    ...
    for dept in CEID_DEPARTMENTS:
        ...
        for display_name, key in VERTICAL_DISPLAY_TO_KEY.items():
            ...
            try:
                sheet_id = _extract_sheet_id(sheet_link)
                sheet_df = load_faculty_vertical_sheet(sheet_id, key)
